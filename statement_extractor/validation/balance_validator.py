...
class BalanceValidator:
    ...
    def validate(self, transactions: List[Transaction]) -> List[Transaction]:
        ...
        for txn in transactions:
            ...
            # Cap absolute tolerance at ₹1.00 to prevent large-balance rows
            # from masking a misassigned small amount (e.g. 120 in wrong column
            # gives error=240, which is within 1% of ₹29,467 but is clearly wrong).
            tol = min(
                self.config.tolerance_fraction * max(abs(txn.balance), 1.0),
                1.00,
            )
            ...
            if err <= 1e-4:
                ...
                txn.validation_status = ValidationStatus.VALIDATED
                txn.confidence_score = min(1.0, txn.confidence_score + 0.05)
            else:
                # Attempt column-swap repair
                swapped_expected = prev_balance
                if txn.debit is not None:
                    swapped_expected += txn.debit
                if txn.credit is not None:
                    swapped_expected -= txn.credit
                
                swapped_err = abs(txn.balance - swapped_expected)
                if swapped_err <= tol:
                    # Pydantic v2-safe mutation via object.__setattr__
                    new_debit = txn.credit
                    new_credit = txn.debit
                    object.__setattr__(txn, 'debit', new_debit)
                    object.__setattr__(txn, 'credit', new_credit)
                    txn.validation_status = ValidationStatus.VALIDATED
                    txn.confidence_score = max(0.0, min(1.0, txn.confidence_score - 0.10))
                else:
