    ...
    def extract(self, file_path: str) -> ExtractionResult:
        ...
        for page_num in range(total_pages):
            ...
            for table_idx, table_rows in enumerate(tables):
                ...
                if not zones:
                    # Before giving up, try to inherit zones from the most recent
                    # successfully-processed transaction table on a nearby page.
                    # This rescues continuation pages whose sparse header rows
                    # don't provide enough column support for DBSCAN to cluster.
                    inherited = getattr(self, '_last_transaction_zones', None)
                    if inherited:
                        import copy
                        zones = copy.deepcopy(inherited)
                        logger.info(
                            "Page %d table %d: column detection failed — "
                            "inheriting %d zones from previous transaction table",
                            page_num + 1, table_idx + 1, len(zones),
                        )
                    else:
                        warnings.append(
                            f"Column detection produced no zones on page {page_num + 1}, "
                            f"table {table_idx + 1}"
                        )
                        continue
                ...
                has_balance = "balance" in roles_found
                has_movement = bool({"debit", "credit", "amount"}.intersection(roles_found))
                has_narration = "narration" in roles_found

                # A table qualifies as a TRANSACTION table if it has:
                #   date + balance + at least one movement column (debit or credit)
                #   OR the original: date + any amount + narration
                # The dual condition handles banks like BOB where the narration and
                # date are in the same column header ("NARRATION DATE"), which
                # causes header inference to assign it the 'date' role, leaving
                # has_narration=False even though it is genuinely a transaction table.
                has_amount = bool({"debit", "credit", "balance", "amount"}.intersection(roles_found))
                is_transaction_table = (
                    (has_date and has_balance and has_movement)
                    or (has_date and has_amount and has_narration)
                )
                ...
                if is_transaction_table:
                    ...
                    # Heuristic: A real transaction table must have descriptions.
                    # If all rows lack descriptions, it's likely a schedule/summary table.
                    if table_txns and not any(t.description.strip() for t in table_txns):
                        logger.info("%s skipped (no descriptions found, likely a summary table)", table_id_str)
                        continue
